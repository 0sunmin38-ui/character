...
### 읽기 전용 배포 (GitHub Pages)

```bash
node pages.mjs      # docs/ 에 정적 사이트 생성 → 커밋 → Settings → Pages 에서 main /docs 선택
```

Pages 는 정적 파일만 서빙하므로 **보는 것만** 된다. 수집·서재 담기·사전 편집은 안 된다.
공개로 나가므로 닉네임까지 뺀 사본을 만들고, `noindex` 와 `robots.txt` 로 검색엔진 수집을 막는다.
...
뷰어 상단 **내보내기** 버튼은 서재에 담은 글을 마크다운 zip 으로 바로 내려준다.
아래 CLI 형식들은 공개용 사본을 만들 때 쓴다.
